      ...
      css.walkRules(function(rule){
        ...
        if (rules && rules.length) {
          rules.forEach(function(e){
            var className = e.replace('.', '')
            if (rule.sourceFile !== 'mixins') {
              classList[className] = {source: rule.sourceFile}
            } else {
              var sourceFile = sourceLookup[e.replace(/\.?(xs\-|sm\-|md\-|lg\-)?/,'')]
              classList[className] = {source: sourceFile}
            }

            if (rule.type == 'rule' && rule.nodes.length) {
              rule.nodes.forEach(function(node){
                if (node.prop.search('color') > -1 && node.prop.search('border') < 0) {
                  classList[className]["color"] = node.value
                }
              })
            }
          })
